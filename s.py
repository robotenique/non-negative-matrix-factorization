...
def create_snack_from_url(url:str, country:str=None, category:str=None):
    ...
    time.sleep(DELAY)
    soup = None
    for soupReq in continuous_request(url):
        # Continuosly asks for requests while it returns null
        if soupReq != None and soupReq.find_all(id="reviewtop") != None:
            soup = soupReq
            break
    if soup == None:
        return None
...
def scrap_snack_list(url:str, country:str=None) -> List:
    ...
    for soupReq in continuous_request(url):
        # Continuosly asks for requests while it returns null
        if soupReq != None and soupReq.find('div', id='longlist') != None:
            soup = soupReq
            break
    if soup == None:
...
def scrap_per_country() -> List:
    ...
    soup = None
    for soupReq in continuous_request(country_link):
        # Continuosly asks for requests while it returns null
        if soupReq != None and soupReq.find('div', class_='triple') != None:
            soup = soupReq
            break
    if soup == None:
        return []
...
"""
Generate maximum of 50 requests to a specific link(generator)
"""
def continuous_request(link):
    max_req = 50
    i = 0
    header = {'User-Agent': rnd.choice(user_agent_list)}
    while i < max_req:
        if i%5:
            header = {'User-Agent': rnd.choice(user_agent_list)}
        req_result = Req.get(link, headers=header).content
        yield BeautifulSoup(req_result, 'html.parser')
        time.sleep(5)
        print(f"\t\t\tTrying new request {i}º time...")
        with open('request_handling.txt', 'a') as the_file:
            the_file.write(f'------------------- NEW REQUEST {i}({link})-------------------- \n')
            the_file.write(req_result)
            the_file.write("\n\n")
        i += 1
